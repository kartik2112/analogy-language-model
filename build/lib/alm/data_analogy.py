...
def get_dataset_raw(data_name: str, cache_dir: str = default_cache_dir_analogy):
    ...
    url = 'https://github.com/asahi417/AnalogyTools/releases/download/0.0.0/analogy_test_dataset_with_prediction.zip'
#     if not os.path.exists('{}/analogy_test_dataset_with_prediction'.format(cache_dir)):
    if not os.path.exists('{}/analogy_test_dataset_with_prediction'.format(cache_dir)):
        open_compressed_file(url, cache_dir)
    assert data_name in ['sat'], 'unknown data: {}'.format(data_name)
    with open('{}/analogy_test_dataset_with_prediction/{}/test.jsonl'.format(cache_dir, data_name), 'r') as f:
        test = list(filter(None, map(lambda x: json.loads(x) if len(x) > 0 else None, f.read().split('\n'))))
#     with open('{}/analogy_test_dataset_with_prediction/{}/valid.jsonl'.format(cache_dir, data_name), 'r') as f:
